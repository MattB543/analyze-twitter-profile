...
(() => {
  if (window.__timelineInterceptorInstalled) {
    console.log("⚠️ PAGE_PATCH: Timeline interceptor already installed, skipping");
    return;
  }
  ...
  console.log("🔍 PAGE_PATCH: Original XMLHttpRequest type:", typeof window.XMLHttpRequest);
  ...
  window.fetch = async function (...args) {
    ...
    if (
      ...
      if (DEBUG) console.log(`📋 PAGE_PATCH: Target endpoint match: ${isTargetEndpoint}`);
      if (isTargetEndpoint) {
      res
        .clone()
        .text()
        .then((txt) => {
          if (DEBUG) console.log("✅ PAGE_PATCH: Captured GraphQL response via fetch:", url);
          relay(txt, url, "fetch");
        })
        .catch(err => { if (DEBUG) console.error("❌ PAGE_PATCH: Failed to process GraphQL response:", err); });
      } else {
        if (DEBUG) console.log("❌ PAGE_PATCH: GraphQL request not captured (no match):", url);
      }
    }
    return res;
  };
  ...
  setTimeout(() => {
    ...
    console.log("🧪 PAGE_PATCH: Current XHR function:", typeof window.XMLHttpRequest);
    console.log("🧪 PAGE_PATCH: Interceptor flag:", window.__timelineInterceptorInstalled);
    ...
    if (DEBUG) {
      ...
      fetch("https://example.com/test").catch(() => {
        console.log("🧪 PAGE_PATCH: Dummy fetch test completed (expected to fail)");
      });
    }
  }, 1000);
})();
